            ...
            window.naver.maps.Service.reverseGeocode({
                ...
                try {
                    ...
                    if (status !== window.naver.maps.Service.Status.OK) {
                        reject(new Error(`역지오코딩 서비스 오류 (status=${status})`));
                        return;
                    }

                    // 응답 구조 체크 - Status.ERROR만 아니라 Status.NO_RESULT 등도 잡을 수 있게 응답 구조 방어 코드 수정
                    if (!response || !response.v2) {
                        reject(new Error('역지오코딩 응답이 비어 있습니다.'));
                        return;
                    }

                    if (!Array.isArray(response.v2.results) || response.v2.results.length === 0) {
